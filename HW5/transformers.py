...
def scaled_dot_product_two_loop_single(
    ...
    args:
        ...
        value: a Tensor of shape (K, M) where K is the sequence length Sand M is
            the sequence embeding dimension
    ...
    for i in range(K):
        weights = torch.zeros(K, dtype=query.dtype)
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):  # loop over batch
        for i in range(K):  # loop over queries

            weights = torch.matmul(key[n], query[n, i]) / math.sqrt(M)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        c_k = math.sqrt(6.0 / (dim_in + dim_q))
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.heads = nn.ModuleList(
            [SelfAttention(dim_in, dim_out, dim_out) for head in range(num_heads)]
        )
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        sd = torch.sqrt(((x - mean) ** 2).mean(dim=-1, keepdim=True))
        norm = (x - mean) / (sd + self.epsilon)
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.multiheadattn = MultiHeadAttention(num_heads=num_heads, dim_in=emb_dim, dim_out=emb_dim // num_heads)

        self.norm1 = LayerNormalization(emb_dim)
        self.norm2 = LayerNormalization(emb_dim)

        self.ff = FeedForwardBlock(inp_dim=emb_dim, hidden_dim_feedforward=feedforward_dim)
...
def get_subsequent_mask(seq):
    ...
    N, K = seq.shape
    mask = torch.triu(torch.ones(K, K, dtype=torch.bool, device=seq.device), diagonal=1)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    positions = torch.arange(K) / K 
    positions = positions.unsqueeze(1).repeat(1, M) 
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    p = torch.arange(K).unsqueeze(1).float()
    i = torch.arange(M).unsqueeze(0).float()
    alpha = torch.floor(2 * torch.floor(i / 2) / M)

    den = torch.pow(10000, alpha)
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        dec_out = dec_out.contiguous().view(-1, dec_out.size(-1))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
